...
src_compile() {
	...
	for cmd in ${CMDS}; do
		einfo "Building ${cmd}..."
		CGO_ENABLED=1 go build -ldflags "${GOLDFLAGS}" -tags stringlabels -o ./dist/${cmd} ./cmd/${cmd}
	done
	for tool in ${TOOLS}; do
		einfo "Building ${cmd}..."
		CGO_ENABLED=1 go build -ldflags "${GOLDFLAGS}" -tags stringlabels -o ./dist/${tool} ./tools/${tool}
	done
}
...
src_install() {
	...
	insinto "/etc/${PN}"
	doins "docs/configurations/single-process-config-blocks.yaml" "mimir-local-config.yaml"
