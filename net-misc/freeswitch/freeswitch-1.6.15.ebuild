...
src_prepare() {
	...
	epatch "${FILESDIR}/freeswitch-automake.patch"
	epatch "${FILESDIR}/freeswitch-minimal-config.patch"
#	sed -e 's/-Werror//' -i configure || die "failed to prepare compile options"
#	sed -e 's/-Wno-unused-result//' -i configure || die "failed to prepare compile options"
...
pkg_postinst() {
	...
	einfo "    http://confluence.freeswitch.org/"
	einfo
}
