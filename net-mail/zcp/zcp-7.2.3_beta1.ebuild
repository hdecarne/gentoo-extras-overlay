...
pkg_postinst() {
	einfo
	einfo "Zarafa has been successfully emerged!"
	einfo
	einfo "Beginnging with ZCP 7.2.2 unprivileged users are used by default."
	einfo "Please make sure that access rights for existing directories are"
	einfo "modifed accordingly for example by issueing the following commands:"
	einfo "    chown -R ${ZARAFA_USER}:root /var/lib/zarafa"
	einfo "    chown -R ${ZARAFA_USER}:root /var/log/zarafa"
	einfo "    chown -R ${ZARAFA_USER}:root /etc/zarafa"
	einfo
	einfo "For more details see the Zarafa changelog at:"
	einfo
	einfo "    https://download.zarafa.com/community/beta/7.2/changelog-7.2.txt"
	einfo
}
