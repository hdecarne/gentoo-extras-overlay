...
IUSE="apache2 berkdb ctypes-python debug doc +dso extras gnome-keyring java kde nls perl python ruby sasl vim-syntax +webdav-serf"
...
pkg_setup() {
	...
	if ! use webdav-serf; then
		...
		ewarn "Consider enabling the USE-flag:"
		ewarn "  webdav-serf"
		echo -ne "\a"
	fi
...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-fix-parallel-build-support-for-perl-bindings.patch
	epatch_user
