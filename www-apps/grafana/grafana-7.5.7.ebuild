...
YARN_OPTIONS="--scripts-prepend-node-path --network-concurrency 1 --network-timeout 3000000"
...
src_prepare() {
	...
	yarn ${YARN_OPTIONS} install --pure-lockfile --no-progress || die "prepare failed"
	default
}
...
src_compile() {
	...
	yarn ${YARN_OPTIONS} run build || die "compile failed"
	yarn ${YARN_OPTIONS} run plugins:build-bundled || die "compile failed"
}
