...
src_compile() {
	...
	go build -ldflags "${GOLDFLAGS}" ./cmd/influx{,d,_stress,_inspect,_tsm} || die "go build failed"
	go install -ldflags "-w -s ${GOLDFLAGS}" ./cmd/influx{,d,_stress,_inspect,_tsm} || die "go install failed"
	use man && emake -C man
}

src_install() {
	dobin "${HOME}/go/bin/influx"
	dobin "${HOME}/go/bin/influxd"
	dobin "${HOME}/go/bin/influx_stress"
	dobin "${HOME}/go/bin/influx_inspect"
	dobin "${HOME}/go/bin/influx_tsm"
